...
internal object WSConnectionBroker {
    ...
    internal suspend fun sendAll(message: WSMessage) {
        _connections.values.forEach {
            it.send(message)
        }
    }

    internal suspend fun send(id: String, message: WSMessage) {
        _connections[id]?.send(message)
    }
}